...
export function calcularPresupuesto(input: {
  ...
  costoPiezas: number;
  tarifas: { basica: number; media: number; compleja: number };
  precioRefNuevo: number;                 // obligatorio
  precioRefSegundaMano?: number;          // opcional
}) {
  const tarifa =
    input.dificultad === "basica" ? input.tarifas.basica :
  ...
  if (total > 0.7 * input.precioRefNuevo) alertas.push("supera_70_por_ciento_nuevo");
  if (input.precioRefSegundaMano != null && total > input.precioRefSegundaMano) alertas.push("supera_precio_segunda_mano");
  if (total >= input.precioRefNuevo) alertas.push("no_viable_vs_nuevo");
